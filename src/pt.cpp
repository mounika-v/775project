...
PathTracer::PathTracer(const std::string &scene_name, bool sample_light, bool volumetric, double sigma_s, double sigma_a) :
	...
	if(scene_name != "") { // if set scene_name, load scene from file
		...
		Sphere *sp8 = new Sphere(5.5, vec3(0,81.6-16.5,81.6),vec3(4,4,4)*100,  vec3(), DIFF);//Lite
...
*/
    ...
    Sphere *medium = new Sphere(300, vec3(10,50,80), vec3(), vec3(), DIFF);
...
vec3 PathTracer::UniformSampleSphere() const {
...
double PathTracer::Scatter(const Ray &r, Ray *sray, double tin, float tout, double &s) const {
	...
	// s=drand48();
						//Sampling with equiangular
						const Shape &light_object = *my_scene_.GetShapes()[8];
						vec3 dir1 = UniformSampleSphere();
						vec3 origin = light_object.GetPosition();
						vec3 p = origin + dir1 * (light_object.radius + 0.1);
						vec3 a = (r.org +r.dir*tin);
						vec3 b = (r.org +r.dir*tout);
						vec3 ap = p - (r.org +r.dir*tin); //p -a
						vec3 ab = (r.org +r.dir*tout) - (r.org +r.dir*tin); //b-a
						vec3 proj = (r.org +r.dir*tin) + (ap.Dot(ab)/ab.Dot(ab)) * ab;
						// std::cout<<"proj:"<<proj.x<<" "<<proj.y<<" "<<proj.z<<std::endl;
						vec3 delta = proj - p;
						vec3 D = proj - p;

						double thetaa = atan ((proj - a).Dot(proj - a)/D.Dot(D));
						double thetab = atan ((b - proj).Dot(b - proj)/D.Dot(D));

						double zeta = drand48();
						// std::cout<<"zeta:"<<zeta<<std::endl;
						vec3 x = D * tan((1-zeta)*thetaa + zeta*thetab);

	// vec3 x = r.org + r.dir *tin + r.dir * s;
	// vec3 x = vec3(drand48(),drand48(),drand48());
