...
vec4 raytracer::render_media(ray& v)
{
	//Rendering the participating medium (homogenous and single scattering)
	// as for t is considered, picking samples between a,b attenuated by 1/r^2 is to be done. 
	// a formula is given in the presentation based on a, b and zeta with zeta being in domain [0,1).
	int tsamples = 10; //no. of points between a, b being considered.
	//For each ray, we obtain D and delta once.
	vec4 a,b,p; // p is the point light
	p = vec4(-3.0,4.5, -1.4);
	a = v.o; b = v.t;
	
	vec4 ap = p - a;
	vec4 ab = b - a;
	vec4  proj = a + ap.dot(ab) / ab.dot(ab)  * ab;
	vec4 delta = proj - v.o;
	vec4 D = proj - p;
	
	float thetaa = atan( (proj - a).length() / D.length())  ;
	float thetab = atan( (b - proj).length() / D.length()) ;
	
	
	
	vec4 flux = vec4 (1.0,1.0,1.0,0.0);
	float att = 0.05; // attenuation constant 0.05 for medium fog
	float scat  = 1; // scattering constant
	vec4 illum(0,0,0);
	for (int i=0; i < tsamples; i++)
	{
		float zeta = randd();
		vec4 sam = D * tan((1-zeta)*thetaa + zeta*thetab);
		vec4 tv = proj - sam;
		float t = tv.length(); //magnitude of tv sqrt (tv.dot(tv))
		float D2t2 = D.dot(D) + tv.dot(tv);
		float eexp =  -att * (t + delta.length() + sqrt(D2t2));
		float expval = exp (eexp);
		float pdft = D.length() / ((thetaa - thetab) * (D2t2));
		illum += ( (flux *= (expval  / D2t2))  *= (1/pdft));
	}
	illum = illum * (scat / tsamples);
	return illum;
}
		...
		for(int j=0; j<image_height; j++)
		{
			...
			for(int m=0; m<samples1D; m++)
			{
				for(int n=0; n<samples1D; n++)
				{
					...
					ray v(org, dir);
					color += (trace_path(v, 0)+render_media(v));
					//dummy += render_media(v);
				}
			}
			color *= 1/samples;
			color.clamp(0.0, 1.0);
